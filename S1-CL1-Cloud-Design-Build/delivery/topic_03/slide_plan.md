# Topic 03 Building the evidence — Slide plan
> **Covers:** Topic 03 — see coverage.md
> **Subtitle:** Options, cost and risk — the case a board can act on

## Slides

### Opener
- [BESPOKE] From diagnosis to evidence
  - A board doesn't fund a problem — it funds a well-evidenced answer.
  - Three moves: lay out the options → cost them over time → weigh the risks.
  - These become the next sections of the same business case.
  - You still don't recommend yet — you assemble the evidence a recommendation rests on.
  kicker: you've proved the problem — now build the answer
  image: none
  notes:
...
- [BESPOKE] You don't recommend until you've compared
  ...
  - Credible advice compares realistic options on a level field.
  - Your job here: lay the options out and evaluate them — not pick one.
  image: none
  notes:
...
- [BESPOKE] Define the workload first
  - Before comparing options, state what any option must satisfy — from your gap analysis + the migration requirements:
    ...
    - data residency & retention · integrations
  - Write it down first — it's the yardstick every option is measured against.
  kicker: options are judged against it
  image: none
  notes:
...
- [BESPOKE] Identify the realistic options
  - Keep the option set honest and small:
    ...
    - Option B — migrate to the cloud
  - Name a third only if it's genuinely real.
  - Every comparison needs the status-quo option — or “moving” has nothing to beat.
  image: none
  notes:
...
- [BESPOKE] Choosing an evaluation method
  - Evaluate each option on two stated lenses:
    ...
    - Difficulty of implementing — effort, disruption, skills, dependencies
  - Score each option on each lens and show your reasoning. (Cost comes next.)
  kicker: criteria, not gut feel
  image: none
  notes:
...
- [BESPOKE] Impact & difficulty — a first read
  ...
  - High impact + high difficulty = real, but plan for it.
  - This is the qualitative shortlist — the CBA and risk sections test it next.
  image: none
  notes:
...
- [EX] Define the options & evaluate
  ...
  - Remember to:
    - State the workload/requirements an option must satisfy (from your gap table + migration reqs).
    - Lay out Option A (renew on-prem) and Option B (migrate to cloud).
    - Pick an evaluation method; assess each option's impact (vs objectives) and difficulty.
  - ~½–1 page.
  - ⚠  Evaluate, don't decide — the recommendation comes later.
...
- Teaches: [ICTICT517 FS Numeracy]
- Kicker: put a number on it
- [BESPOKE] Put a number on it
  - A CBA compares the options in money, over time — here, 5 years.
  - It turns “cloud feels cheaper” into evidence — or disproves it.
  - The board decides on total cost + benefit, not a monthly price.
  image: none
  notes:
...
- [BESPOKE] Costing the on-prem option (Option A)
  ...
  - Watch software licensing (Windows / SQL Server) — it rides on BOTH options; don't drop it.
  image: none
  notes:
...
- [BESPOKE] AWS Pricing Calculator
  - Use it to:
    ...
    - model a solution before building it
    - find instance types & contract terms; group services into an estimate
  - calculator.aws
  kicker: size the cloud option before you build it
  image: reuse 03-pricing-calculator.png
  notes:
...
- [EX] Build a cost estimate
  ...
  - Remember to:
    - Size the Accounting System's compute, storage, database and data transfer.
    - Name your estimate; group the services; capture the monthly + annual figures.
    - Be ready to report your findings back to the class.
  - This estimate feeds your CBA (Option B).
  - 🔗  calculator.aws
  timer: ~20 min, then report back
  image: none
  notes:
...
### C3 — Risk & impact
- Teaches: [ICTICT517 PC 2.1]
- Kicker: what money doesn't capture
- [BESPOKE] Money isn't the whole story
  - Two options can cost the same and carry very different risk.
  - Risk & Impact is the qualitative complement to the CBA:
    - the intangibles, and the things that could go wrong
  image: none
  notes:
...
- [BESPOKE] Intangibles comparison
  - Name the qualitative factors, per option:
    ...
    - security posture · business continuity · disruption during change
  - The cheaper option may carry the bigger intangible cost.
  kicker: what a dollar figure misses
  image: none
  notes:
...
- [BESPOKE] The risk register
  ...
  - Keep it specific to this migration: data-migration error, cutover downtime, cost overrun, skills gap.
  - A risk with no mitigation is just a worry; a mitigation makes it manageable.
  image: none
  notes:
...
- [EX] Add the Risk & Impact assessment
  ...
  - ~½–1 page.
  - ⚠  Specific risks with real mitigations — not generic “things might go wrong”.
  timer: ~20 min, then we discuss
  image: none
  notes:
...
### Close
- [BESPOKE] From evidence to decision
