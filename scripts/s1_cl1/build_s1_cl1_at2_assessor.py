...
MARKING = [
    ['A2 §4.1 + §4.2 + §4.7 Build narrative — Foundation tier (IAM, network, security) — student describes the IAM groups/users/MFA enforcement, the VPC + subnets + gateways + routing, and the security-group model, and identifies the shared security responsibility outcome of the build; cross-references to the Appendix A screenshots', '[ICTCLD401 PC 1.2] · [ICTCLD401 PC 1.4] · [ICTCLD401 PC 1.5] · [ICTCLD401 PC 1.6] · [ICTCLD401 PC 1.7] · [ICTCLD401 PC 2.1] · [ICTCLD401 PC 2.2] · [ICTCLD502 PC 1.3]'],
    ['A3 §4.3 + §4.4 + §4.5 + §4.6 Build narrative — Workload tier (compute, load balancing, database, storage) — student describes the EC2 + ALB + ASG, the RDS managed database, and the EBS + S3 storage; cross-references to the Appendix A screenshots', '[ICTCLD401 PC 2.3] · [ICTCLD401 PC 2.4] · [ICTCLD401 PC 2.5] · [ICTCLD502 PC 4.1]'],
    ['A4 §4.8 Build narrative — Operability (autoscaling configuration, baseline CloudWatch monitoring) — student describes the ASG scaling policy + the baseline CloudWatch alarms', '[ICTCLD401 PC 3.1] · [ICTCLD502 PC 4.3] (monitoring baseline only — full availability monitoring is AT3)'],
    ['A5 §5 Configuration Decisions — for each of the two decisions the supplied design leaves open, student names at least two options considered, states the choice made, and justifies it against the YAT LMS workload (referencing the LMS Application Specification), not generically', '[ICTCLD401 PC 1.1] (options considered and compared) · [ICTCLD401 PC 1.3] (best option selected)'],
    ['A6 §6 Testing and Validation — student ran all five tests (connect to the application server; reach the internet; reach the database; reach the load balancer; automatic scaling), with the required screenshot pasted into each test box and any failure and its fix noted', '[ICTCLD401 PC 2.6] · [ICTCLD401 PC 3.2] · [ICTCLD401 PE 3] · [ICTCLD502 PC 4.2]'],
...
PROSE_PART2_TIPS = [
    ...
    ('Complete every section of the Deployment Report template that asks you for a response, and populate every appendix. The template provides explicit prompts in each section — follow them. §2 and §3 are already written for you; leave them as supplied and delete the violet note above each.', 'Assessor text'),
    ('Your report covers:', 'Assessor text'),
    ('§2 Engagement Context and §3 Scope of Deployment (supplied — already written for you)', 'item'),
    ('§4 Build Narrative (chronological account by layer — IAM, network, compute, ALB, DB, storage, security, monitoring)', 'item'),
    ('§5 Configuration Decisions (the two the design leaves open — options considered, your choice, and why)', 'item'),
    ...
    ("For each Configuration Decision, name the alternative you considered as well as the one you chose. A choice with no alternative beside it is not a comparison.", 'Assessor text'),
...
ASSESSOR_BODY = [
    ...
    ('p', 'There are no supporting sections — every section of the report is covered by a criterion.'),
    ('p', '§2 Engagement Context + §3 Scope of Deployment are supplied in the template — already written, and not student work. They are not marked. A student who has edited or removed them has departed from the supplied report; treat that under A13 (Document Quality).'),
    ...
    ('p', 'Cross-references A4, A5, A6, A7'),
    ...
    ('p', 'Cross-references A8'),
    ('p', '5.7 §4.8 Monitoring baseline (criterion A6)'),
    ('p', "Both baseline CloudWatch alarms named and evidenced (per the supplied design's §4.10 list): ALB target health status, and RDS free storage low"),
    ...
    ('p', 'Satisfactory looks like: both decisions carry at least two options considered, a stated choice, and a rationale tied to the YAT LMS workload (not generic):'),
    ('tbl', [
        ['#', 'Decision', 'What a credible justification references'],
        ['C1', 'Application-tier instance type', 'Concurrent-user load from the LMS application spec (200–300 typical, 500–700 peak); growth headroom'],
        ['C2', 'Database instance class and storage size', 'Database workload characteristics (read-heavy for an LMS); the current data footprint and its growth rate'],
    ]),
    ...
    ('p', 'NYS:'),
    ('p', 'Either decision left blank or with "TBD"'),
    ('p', 'Only the chosen option given, with no alternative considered — the comparison is not evidenced'),
    ...
    ('p', 'UoC evidenced: [ICTCLD401 PC 2.6, 3.2] · [ICTCLD401 PE 3] · [ICTCLD502 PC 4.2]. Critical section.'),
    ('p', 'Each test is instrumented: the template gives the student the steps and a box to paste the screenshot into. Mark the screenshot, not the prose.'),
    ('tbl', [
        ['Test', 'Satisfactory evidence in the box'],
        ['6.1 Connect to the application server', 'A terminal session on the instance, connection succeeding'],
        ['6.2 Reach the internet', 'The command and an HTTP response header returned through the NAT gateway'],
        ['6.3 Reach the database', 'The command and output confirming port 3306 reachable from the app tier'],
        ['6.4 Reach the load balancer', 'The command and an HTTP response from the load balancer'],
        ['6.5 Automatic scaling', 'The Auto Scaling Activity tab showing both a scale-out and a scale-in entry'],
    ]),
    ('p', 'NYS:'),
    ('p', 'Any screenshot box left empty'),
    ('p', 'A screenshot that does not show what the box asks for'),
    ('p', 'Evidence that contradicts the claimed outcome'),
    ('p', 'Screenshots from another environment rather than the student\'s own build'),
    ...
    ('p', 'Model coverage: Student explains that the browser cannot reach anything by name alone — DNS translates the LMS hostname into an address the browser can connect to, and in this deployment that address is the load balancer.'),
    ('p', "The LMS hostname resolves (via YAT's authoritative DNS) to the load balancer's DNS name, which AWS in turn resolves to one of the load balancer's current IP addresses."),
    ('p', 'Because those addresses change, the name is what stays stable — which is why the hostname points at the load balancer rather than at any instance.'),
    ('p', "If misconfigured, the browser gets the wrong address or none at all, and staff cannot reach the LMS even though every server behind it is running."),
    ('p', 'Accept any answer that shows DNS is the name-to-address step and names the load balancer as the target. A student who also mentions the instance resolving the database endpoint has gone beyond the question — credit it, do not require it.'),
    ('p', 'NYS: generic "DNS resolves names to IPs" with no reference to their own deployment.'),
    ...
    ('p', '13. Marking A13 — Document quality'),
    ('p', 'UoC evidenced: [ICTCLD401 FS Writing] · [ICTCLD401 FS Reading] · [ICTCLD502 FS Reading].'),
    ('p', 'Satisfactory looks like:'),
    ('p', 'Plain English, with technical vocabulary used accurately and explained where the reader would need it'),
    ('p', 'Structure and formatting consistent with a professional consulting deliverable for an RTO client'),
    ('p', 'The supplied design and the LMS application specification have been read correctly — the report does not contradict them on a point of fact'),
    ('p', 'The supplied §2 and §3 are present and unedited'),
    ('p', 'NYS:'),
    ('p', 'Hard to follow, or reads as working notes rather than a deliverable'),
    ('p', 'Technical terms used incorrectly, indicating the supplied documentation was not understood'),
    ('p', 'Contradicts the supplied design or the application specification'),
    ...
    ('tbl', [
        ...
        ['PC 1.2 — Identify impact of shared security responsibility models', 'A2'],
        ...
        ['PE 2 — Configure compute, storage, database and autoscaling resources within virtual network', 'A9'],
        ['PE 3 — Conduct simple tests to confirm access to resources', 'A6'],
    ...
    ('tbl', [
        ...
        ['PC 4.2 — Demonstrate connectivity between resources at all tiers', 'A6'],
        ['PC 4.3 — Monitor and measure availability of resources', 'A4 (baseline monitoring only — HA-tuned monitoring is AT3)'],
        ['FS Reading', 'A13'],
        ['AC 1 — Cloud vendor service provider', 'C1 (pre-condition)'],
        ['AC 2 — Cloud managed database service', 'C1 (pre-condition)'],
