...
written Deployment Report (with three evidence appendices) documenting the implementation of a
...
OVERVIEW = [
    ...
    'The assessment is a single-task project delivered as one written artefact (the Deployment Report) with three populated appendices. Implementation occurs in the AWS Academy lab environment authorised for this cluster. There is no presentation or observation event — all assessment evidence is captured in the Deployment Report and its appendices.',
    ...
    'Submission: the completed Deployment Report (.docx) with all three appendices populated is submitted via the LMS.',
...
TASKS = [
    ...
    "Produce a Deployment Report documenting the build, using the YAT-provided Deployment Report template. The report covers what was built, configuration decisions made where the supplied design left them open, testing and validation outcomes, an operational handover for YAT IT, written responses to six contextual Knowledge Evidence questions about the student's own build, and three appendices of evidence.",
    "The Deployment Report is the single submitted deliverable. All build evidence (screenshots, test results, reflections) is captured in the report's appendices — there is no separate portfolio submission and no presentation event.",
...
ASSESSOR_BODY = [
    ...
    ('p', 'Supporting sections (where NYS may be balanced against compensating evidence elsewhere): Appendix D Reflections (A12).'),
